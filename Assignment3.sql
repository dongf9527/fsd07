...
answer: SELECT b.username AS user_one_name,c.username user_two_name
FROM `relationship` a
...
answer: SELECT b.username AS user_one_name,c.username user_two_name,`status`,d.username AS action_user_name
FROM `relationship` a
LEFT JOIN `users` b ON a.`user_one_id`=b.`user_id`
LEFT JOIN `users` c ON a.`user_two_id`=c.`user_id`
